...
class DeadL:
    ...
    def draw_circle(cls, frame, pose_predict, landmarks):
        ...
        right_wrist = landmarks.landmark[15]
        left_wrist = landmarks.landmark[16]
        if pose_predict > cls._enter_threshold:
            frame = drawing.image_alpha(frame, right_wrist.x * frame_width, right_wrist.y * frame_height, 30, (0, 255, 0),
                                        0.3,
                                        1, 1)
            frame = drawing.image_alpha(frame, left_wrist.x * frame_width, left_wrist.y * frame_height, 30, (0, 255, 0),
                                        0.3, 1,
                                        1)
        elif pose_predict > cls._exit_threshold:
            frame = drawing.image_alpha(frame, right_wrist.x * frame_width, right_wrist.y * frame_height, 30, (0, 255, 255),
                                        0.3, pose_predict - cls._exit_threshold,
                                        cls._enter_threshold - cls._exit_threshold)
            frame = drawing.image_alpha(frame, left_wrist.x * frame_width, left_wrist.y * frame_height, 30, (0, 255, 255),
                                        0.3, pose_predict - cls._exit_threshold,
                                        cls._enter_threshold - cls._exit_threshold)
        else:
            frame = drawing.image_alpha(frame, right_wrist.x * frame_width, right_wrist.y * frame_height, 30,
                                        (255, 255, 255),
                                        0.3, 1, 1)
            frame = drawing.image_alpha(frame, left_wrist.x * frame_width, left_wrist.y * frame_height, 30, (255, 255, 255),
                                        0.3, 1, 1)
