...
class Track:
    ...
    def analyze_image(self, file_path, model, framework, resolution, lite):
        """
    ...
    def analyze_video(self, file_path, model, framework, resolution, lite):
        """
    ...
    def analyze(self, video, file_path, model, framework, resolution, lite):
        """
        ...
        if file_path is None:
            coordinates = self.analyze_camera(model, framework, resolution, lite)

        # Video analysis
        elif video:
            coordinates = self.analyze_video(file_path, model, framework, resolution, lite)

        # Image analysis
        else:
            coordinates = self.analyze_image(file_path, model, framework, resolution, lite)

        return coordinates

    def annotate_image(self, file_path, coordinates):
        """
    ...
    def annotate_video(self, file_path, coordinates):
        """
    ...
    def annotate(self, video, file_path, coordinates):
        """
        ...
        if video:
            coordinates = self.annotate_video(file_path, coordinates)

        # Annotate image predictions
        else:
            coordinates = self.annotate_image(file_path, coordinates)

    def save(self, video, file_path, coordinates):
        """
        ...
        # Initialize CSV
        import csv
        csv_path = normpath(file_path.split('.')[0] + '_coordinates.csv') if file_path is not None else normpath(
    ...
    def perform_tracking(self, video, file_path, model_name, framework_name, visualize, store):
        """
        ...
        # PERFORM INFERENCE
        coordinates = self.analyze(video, file_path, model, framework, resolution, lite)

        # VISUALIZE PREDICTIONS
        if visualize and file_path is not None and coordinates:
            self.annotate(video, file_path, coordinates)


        # STORE PREDICTIONS AS CSV
        if store and coordinates:
            self.save(video, file_path, coordinates)

        return True

    def main(self, file_path, model_name, framework_name, visualize, store, mode):
        """
        ...
        if file_path is None:
            ...
            print('##########################################################################################################\n')
            self.perform_tracking(video=True, file_path=None, model_name=model_name, framework_name=framework_name, visualize=False, store=store)

        # VIDEO ANALYSIS
        elif 'Video' in [track.track_type for track in MediaInfo.parse(file_path).tracks]:
            self.perform_tracking(video=True, file_path=normpath(file_path), model_name=model_name, framework_name=framework_name, visualize=visualize, store=store)

        # IMAGE ANALYSIS
        elif 'Image' in [track.track_type for track in MediaInfo.parse(file_path).tracks]:
            self.perform_tracking(video=False, file_path=normpath(file_path), model_name=model_name, framework_name=framework_name, visualize=visualize, store=store)
...
if __name__== '__main__':
    ...
    print('##########################################################################################################\n')
    track = Track()
    track.main(file_path=file_path, model_name=model_name, framework_name=framework_name, visualize=visualize, store=store, mode=mode)